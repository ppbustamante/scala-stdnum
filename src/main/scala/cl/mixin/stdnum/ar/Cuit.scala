...
object Cuit {
  ...
  /** Check if the number is a valid CUIT. This checks the length, formatting
    * and check digit.
    */
  def validate(number: String): Either[ValidationError, String] =
    ...
    else if !CUIT_TYPES.contains(compatNumber.take(2)) then
      Left(InvalidComponent())
    else if this.calcCheckDigit(number) != compatNumber.last then
  ...
  private def calcCheckDigit(number: String): Char = 
    ...
    val check = weights.zip(number).map((w, n) => w * n.toInt).reduce( _ + _) % 11
